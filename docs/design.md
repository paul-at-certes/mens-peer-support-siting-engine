...
3. **Concordance with existing provision.** Does the surface, independently, rate the places a charity already chose by other means? AMC's 354 groups are a decade of local judgement the model has never seen.
   *Implemented (`spikes/group_need_concordance.py`, [ADR 0003](adr/0003-concordance-with-existing-groups.md)).* It must use `need_index`, never `priority_score`, because the supply surface is built from those same groups — the same 292 England & Wales venue areas sit at the 68th percentile of need and the 25th of priority. Headline: **within a local authority** the venue's own small area averages the **64.1st percentile** of need against a null of 50 (p < 5e-05, 20,000 draws), and all three components clear the null separately. **Between local authorities** the apparent agreement is entirely regional — permuting within region leaves it at p = 0.21, because AMC's heartland regions are saturated (North East 12/12 LAs, Yorkshire and The Humber 21/21). So the surface is corroborated about *which part of a town*, and **untested about which town** — which is precisely the judgement a national shortlist is asked for, and therefore what check #1 is for.
4. **Back-testing (not yet possible).** Where groups have already opened, do the areas the model would have prioritised correspond to where new groups subsequently sustained good attendance? This is the closest thing to an outcome test and the most honest measure of whether the index tracks real demand. It needs **opening dates and attendance figures**, which no open source carries — only AMC could supply them. Check #3 is the contemporaneous shadow of this, and is not a substitute for it.
...
- **A rank names a pocket; a group serves a catchment.** Small-area ranking finds the sharpest concentration of need, but a group opened there draws from the surrounding area, which is blander. Measured on existing provision: venue small areas average the 68th percentile of need nationally while the areas they are actually nearest to average the 58th — a 10-point drop ([ADR 0003](adr/0003-concordance-with-existing-groups.md)). Read a high rank as "there is a concentration here", not as "everyone this group reaches looks like this".
